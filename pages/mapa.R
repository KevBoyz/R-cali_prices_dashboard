  ...
  div(
    ...
    div(
      ...
      div(
        ...
        sliderInput(
          "map_age_range",
          "Idade mediana dos imóveis (anos):",
      ...
      div(
        ...
        div(
          style = "padding:14px; border-top:1px solid #e2e8f0;",
          uiOutput("mapa_stats")
        )
      )
    )
...
mapa_server <- function(input, output, session) {
  ...
  dados_filtrados <- reactive({
    req(
      input$map_ocean,
      input$map_value_range,
      input$map_income_range,
      input$map_age_range
    )
  ...
  observe({
    ...
    proxy <- leafletProxy("mapa_leaflet") %>%
      clearMarkers() %>%
      clearControls()
    ...
    popups <- paste0(
      ...
      "<b>total_bedrooms:</b> ",
      ifelse(is.na(df$total_bedrooms), "N/A", format(df$total_bedrooms, big.mark = ",")),
      "<br>",
  ...
  output$mapa_stats <- renderUI({
    ...
    stat_card <- function(label, value) {
      div(
        ...
        div(
          style = "font-size:0.74em; color:#64748b; margin-bottom:3px; text-align:center;",
          label
        ),
        div(
          style = "font-size:0.92em; font-weight:bold; color:#1a2847;",
          value
        )
      )
    }
    ...
    caption <- if (is.null(input$mapa_leaflet_bounds)) {
      ...
      sprintf(
        "Área visível: %s blocos (de %s filtrados)",
        format(n_view, big.mark = ","),
        format(n_filt, big.mark = ",")
      )
    }

    div(
      div(
        style = "font-size:0.82em; color:#64748b; margin-bottom:10px;",
        caption
      ),
      div(
        ...
        stat_card(
          "Idade med. — média",
          paste0(fmt_num(mean(df_view$housing_median_age, na.rm = TRUE)), " anos")
        ),
        stat_card(
          "População total",
          format(sum(df_view$population, na.rm = TRUE), big.mark = ",")
        ),
        stat_card(
          "Domicílios total",
          format(sum(df_view$households, na.rm = TRUE), big.mark = ",")
        )
      )
    )
  })
