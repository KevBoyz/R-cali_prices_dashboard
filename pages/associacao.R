...
associacao_server <- function(input, output, session) {
  ...
  output$plot_corr <- renderPlot({
    ...
    image(
      ...
      zlim = c(-1, 1),
      axes = FALSE,
      main = paste("Correlação de", method_label),
      xlab = "",
      ylab = ""
    )
      ...
      for (i in seq_len(nr))
        for (j in seq_len(nc)) {
          ...
          text(
            ...
            nr + 1 - i,
            labels = round(val, 2),
            col = if (abs(val) > 0.5) "white" else "#333333",
            cex = 0.85
          )
        }
    }
  ...
  output$tabela_contingencia <- renderUI({
    ...
    if (isTRUE(input$cont_total))
      tab <- addmargins(tab)

    nr <- nrow(tab)
    nc <- ncol(tab)
    row_nms <- rownames(tab)
    col_nms <- colnames(tab)

    body_vals <- tab[row_nms != "Sum", col_nms != "Sum", drop = FALSE]
    max_val <- max(body_vals, 1)

    cell_bg <- function(val, is_total) {
      if (!isTRUE(input$cont_heatmap) || is_total)
        return("")
    ...
    body_rows <- sapply(seq_len(nr), function(i) {
      ...
      sprintf(
        '<tr style="%s%s"><th style="%s%s%s">%s</th>%s</tr>',
        row_bg,
        row_fw,
        th_style,
        row_bg,
    ...
    HTML(paste0(
      ...
      sprintf(
        '<p style="font-size:0.82em; color:#888; margin-bottom:6px;">A: %s &nbsp;|&nbsp; B: %s</p>',
        input$cont_varA,
        input$cont_varB
      ),
  ...
  output$resultado_chi <- renderUI({
    ...
    if (input$chi_varA == input$chi_varB) {
      return(div(
        style = "padding:14px;",
        div(
          style = "border-left:4px solid #b45309; padding:10px;
    ...
    if (is.null(expected)) {
      return(div(
        style = "padding:14px; color:#b91c1c;",
    ...
    if (!premissa_ok) {
      ...
      return(div(
        style = "padding:14px;",
        div(
          style = "border-left:4px solid #b91c1c; padding:10px;
                   background:#fef2f2; border-radius:4px;
                   color:#991b1b; font-size:0.88em;",
    ...
    conclusao <- if (rejeitar) {
      sprintf(
        "Rejeita-se H₀ (p = %.4f < α = %.2f): há evidência de associação entre as variáveis.",
        p_val,
        alpha
      )
    } else {
      sprintf(
        "Não se rejeita H₀ (p = %.4f ≥ α = %.2f): sem evidência de associação.",
        p_val,
        alpha
      )
    }
    ...
    div(
      ...
      div(
        style = "color:#166534; font-weight:bold; margin-bottom:12px;",
        "✔ Premissa atendida"
      ),
      tags$table(
        ...
        tags$tr(
          ...
          tags$td(
            style = gsub("border-bottom:1px solid #eee;", "", td2),
      ...
      div(
        style = sprintf(
          "border-left:4px solid %s; padding:10px; background:%s;
